...
print("\n--- Ejercicio 3: Monitoreo y Agregación de Datos con Pipeline de MongoDB ---")

def obtener_datos_ciudad(ciudad):
    """
    Obtiene datos meteorológicos para una ciudad específica de la API de OpenWeatherMap.
    
    Args:
        ciudad (str): Nombre de la ciudad a consultar
        
    Returns:
        dict: Diccionario con los datos meteorológicos procesados
    """
    params = {
        'q': ciudad,
        'appid': API_KEY,
...
def monitorear_ciudad(ciudad, duracion_minutos=30, intervalo_segundos=120):
    """
    Monitorea una ciudad durante un período específico.
    
    Args:
        ciudad (str): Nombre de la ciudad a monitorear
        duracion_minutos (int): Duración total del monitoreo en minutos
        intervalo_segundos (int): Intervalo entre consultas en segundos
    """
    try:
        client = MongoClient(MONGO_CONNECTION_STRING, serverSelectionTimeoutMS=5000)
        client.admin.command('ping')
        ...
        for i in range(num_iteraciones):
            ...
            # Obtener datos
            datos = obtener_datos_ciudad(ciudad)
            if datos:
                # Insertar en MongoDB
...
def analizar_datos_monitoreo():
    """
    Analiza los datos recolectados usando una pipeline de agregación de MongoDB.
    """
    try:
        client = MongoClient(MONGO_CONNECTION_STRING, serverSelectionTimeoutMS=5000)
        client.admin.command('ping')
...
# Ejecutar monitoreo y análisis
print("\nIniciando monitoreo de Bogotá...")
monitorear_ciudad('Bogota', duracion_minutos=30, intervalo_segundos=120)

print("\nAnalizando datos recolectados...")
analizar_datos_monitoreo()
